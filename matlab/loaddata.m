...
clear GLOBAL
global Q2 fileloaded Result Bornes1 L1 
...
if ~isempty(filename) && exist(filename)~=0
    ...
    poub=length(Q(:))/1024
     ...
     handles.databrut=Q2;
     Result = extract_data(Q2(:,1:poub),1,0);
...
% taille_fichier=1024
%  N=floor(length_data/taille_fichier);
% 
% 
% % %==================================================================%
...
% 
% Result.AD_Satellite_label={'Time','HDOP','VDOP','3DFIX','Compteur100hz'};
% Result.AD_Satellite = zeros(N,5);
% %==================================================================%
...
% Result.Pressures = zeros(N*10,28);   % Données a 1000Hz
% % end
% 
% %==================================================================%
% % Modif MIRIAD PressionBaro1, PressionBaro2, PressionPitot         %
...
% 
% 
% % % stockage des donn�es a traiter
% % q=Q(:,offset:size(Q,2)-offset2);
% % q=q.';
% %% Satellite status
% a=[q(:,486) q(:,487) q(:,488) q(:,489)];
% b=dec2single(a);
% Result.AD_Satellite(:,2)=b;
% 
% a=[q(:,490) q(:,491) q(:,492) q(:,493)];
% b=dec2single(a);
% Result.AD_Satellite(:,3)=b;
% a=[q(:,3) q(:,4)];
% b=a(:,2)*256+a(:,1);
% b_bin=dec2bin(b,16)-'0';
% b_bin2=flip(b_bin(:,10:12),2);
% % if b_bin2(2)==1
% Result.AD_Satellite(:,4)=b_bin2(:,2);
% % else
% %     Result.AD_Satellite(:,4)=0;
% % end
...
% b=dec2double(a);
% Result.AD_NAVIGATION(:,6)=b*180/pi;   % Latitude(Rad)
% a=[q(:,21) q(:,22) q(:,23) q(:,24) q(:,25) q(:,26) q(:,27) q(:,28)];
% b=dec2double(a);
% Result.AD_NAVIGATION(:,7)=b*180/pi;   % Longitude(Rad)
...
% Result.AD_Satellite(:,1)=Compteur;
% 
% Compteur2=repmat(Compteur,1,10);
% Compteur2=Compteur2.';
...
% Result.AD_Satellite(:,end)=Compteur;
