...
% correction ecriture ds SD Pacquet donnees %
% Test de positions des patterns: 509 510 511 FF BB BB pour premier paquet. 
% Test 
...
% % Repositionnement des paquets %
% 
 for line=1: N
     
     if q(line,511) == 187 && q(line,512)== 187 && q(line,1011) == 171 && q(line,1012)== 205 && q(line,294) == 51 && q(line,295)== 51 && q(line,494) == 192 && q(line,495)== 1
         
         % Permutation des block invers�s  %    
     elseif q(line,1023) == 187 && q(line,1024)== 187 && q(line,499) == 171 && q(line,500)== 205 % detection des paquets inverses
         Tampon = q(line,1:512); % Valeures Motus   % variable Tempo pour echanger de place les paquets de 512
         q(line,1:512)=q(line,513:1024);
         q(line,513:1024)= Tampon;
          
         
         % doublement block turbulence %
     elseif q(line,511) == 187 && q(line,512)== 187 && q(line,1023) == 187 && q(line,1024)== 187
        q(line,513:1024)=Motus;
         VecteurTest2=[VecteurTest2, line];
         
         % Les lignes � supprimer %
     else
          VecteurTest3=[VecteurTest3, line];
     end     
   Motus= q(line,513:1024);
     
 end

VecteurTest3([1])=[];

save ('VecteurTest.mat','VecteurTest3');
...
Result_adnav=repmat(((Result.AD_NAVIGATION(:,4)*1e6)+ Result.AD_NAVIGATION(:,5))./1e3,1,10)+repmat(0:1:9,N,1);
...
Vecteurx10= VecteurTest3*10;
    
% for i=1 : 52
%     for cpt=0 : 9
%         VecteurP=[VecteurP, Vecteurx10([i])-cpt];
%     end
% end

save ('data.mat');

Result.Pattern(VecteurTest3,:) = [];
Result.AD_NAVIGATION(VecteurTest3,:) = [];
Result.PaquetAirData(VecteurTest3,:) = [];
Result.PaquetWind(VecteurTest3,:) = [];
Result.AirDataSensors(VecteurTest3,:) = [];
Result.IMU(VecteurTest3,:) = [];
Result.MOTUSRAW(VecteurP,:) = [];
Result.MOTUSORI(VecteurTest3,:) = [];
Result.Pressures(VecteurP,:) = [];
Result.TH(VecteurTest3,:) = [];
Result.T2(VecteurTest3,:) = [];
