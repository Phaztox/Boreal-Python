...
rimp=find(abs(Psi-300)<= 5 );  % Heading angle limits
...
kpair=find(abs(Psi-90)<= 6 );  % Heading angle limits
rpair=find(abs(Psi-100)<= 5 );  % Heading angle limits
spair=find(abs(Psi-74)<= 6 );  % Heading angle limits
upair=find(abs(Psi-40)<= 5 );  % Heading angle limits
ypair=find(abs(Psi-40)<= 5 );  % Heading angle limits
...
hpair=find(abs(Psi-25)<= 3 );  % Heading angle limits
...
mpairdebpair1=mpair(aa);
mpairdebpair2=mpair(aa+1);
